...
async function loadTools() {
  ...
  try {
    ...
    const statuses = (await api.get<McpServerStatus[]>('/agent-tools/mcp-status')).data
    if (current === requestVersion) { catalog.value = value; mcpStatuses.value = statuses }
  } catch (reason: any) {
...
<template>
  <section class="tools-page page-reveal">
    ...
    <div v-else-if="catalog" class="tool-groups" :class="{ refreshing: loading }" aria-live="polite">
      ...
      <section class="mcp-section"><header><div><p class="eyebrow">DEPLOYMENT APPROVED</p><h3>MCP 工具</h3></div><span>{{ mcpTools.length }}</span></header><div v-if="mcpStatuses.length" class="mcp-statuses"><span v-for="status in mcpStatuses" :key="`${status.name}-${status.transport}`" :class="{ failed: !status.connected }">{{ status.name }} · {{ status.connected ? '已连接' : status.failure || '未连接' }}</span></div><div v-if="mcpTools.length" class="tool-grid"><article v-for="tool in mcpTools" :key="tool.name" class="tool-card" :class="{ unavailable: tool.enabledForSkill === false }"><header><span class="tool-index">M</span><div><b>{{ tool.displayName }}</b><code>{{ tool.name }}</code></div><em v-if="tool.enabledForSkill === true">已授权</em><em v-else-if="tool.enabledForSkill === false" class="denied">未授权</em><em v-else>已注册</em></header><p>{{ tool.description }}</p><small v-if="tool.disabledReason === 'PERSONAL_SKILL_LOCAL_ONLY'" class="tool-reason">{{ unavailableText(tool) }}</small><details><summary>查看输入协议</summary><pre>{{ formattedSchema(tool) }}</pre></details></article></div><div v-else class="mcp-empty"><b>{{ mcpEnabled ? '当前没有已注册的 MCP 工具' : 'MCP 工具尚未启用' }}</b><p>{{ mcpEnabled ? '部署批准的服务不可用或没有通过只读 allowlist。' : '本地知识问答不受影响；启用后这里只展示已连接且通过白名单的工具。' }}</p></div></section>
