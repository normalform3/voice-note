...
public class OutboxDispatchState {
    ...
    @Transactional(readOnly = true) public Optional<OutboxEvent> load(String id) { return events.findById(id); }
    ...
    @Transactional public void markFailed(String id) { events.findById(id).ifPresent(event -> { if (event.getStatus() == OutboxStatus.READY) { event.markFailed(); events.save(event); } }); }
}
