...
public class RocketMqMessagePublisher implements MessagePublisher {
    ...
    @Override public void publish(OutboxEvent event) {
        String topic = switch (event.getEventType()) {
            case TRANSCRIPTION_REQUESTED, RECORDING_FINALIZATION_REQUESTED, PROGRESS_CHANGED -> properties.getRocketmq().getTranscriptionTopic();
            ...
            case ANALYSIS_REQUESTED, SPEAKER_CORRECTION_REQUESTED, KNOWLEDGE_RUN_REQUESTED,
                 MEMORY_EXTRACTION_REQUESTED, CONVERSATION_SUMMARY_REQUESTED,
                 USER_MEMORY_INDEX_REQUESTED, USER_MEMORY_DELETE_REQUESTED -> properties.getRocketmq().getAnalysisTopic();
        };
