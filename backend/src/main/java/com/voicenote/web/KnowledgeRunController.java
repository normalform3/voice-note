...
public class KnowledgeRunController {
    ...
    private EvidenceView view(KnowledgeRunEvidence value) {
        ...
        if (taskId == null && value.getKnowledgeDocumentId() != null) taskId = documents.findById(value.getKnowledgeDocumentId()).map(document -> document.getTranscriptionTaskId()).orElse(null);
