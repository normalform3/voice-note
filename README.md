...
当前版本的主链路是：上传音频 → 原始文档（完整转写）→ 手动生成正式文档 → 手动建立知识索引 → 证据可回跳的跨文档问答。项目正在持续开发，外部 ASR、向量检索和模型能力需要按环境单独启用。
...
| 原始文档 | 听记完成后保存完整规范化转写、时间轴、说话人 ID，并在对象存储保留 ASR 原始结果文件。 |
| 正式文档 | 用户确认后生成清洗整理文档及其区块，保留其来源转写段，供摘要和知识构建使用。 |
...
2. 上传完成后创建听记任务。页面会展示 ASR 和原始文档的进度；失败阶段可按阶段重试。
3. 在音频详情页阅读原始文档，按需填写说话人名称，再手动生成正式文档。
4. 正式文档完成后，手动建立知识库；完成后可在右侧输入跨文档问题。
